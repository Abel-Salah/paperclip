...
Use choice questions only when there are at least two distinct, meaningful choices. This applies to both single-select and multi-select questions. Do not invent a lone "I'll describe it" or "Provide answer" option to force a text response.

For open-ended questions, use the canonical question format with `answerMode: "text"`, without options or `customAnswer`. With the interaction REST API, put that presentation in `payload.questionSet` (`schema: "paperclip.question_set.v1"`) and include the matching legacy `payload.questions` entries required by the API. The Paperclip API reference's open-ended question example shows both fields. The canonical presentation is what the user sees. With a native question tool, use its supported text-answer mode. Do not send `type: "text"` as a legacy question field.

Keep questions specific to missing information, and use facts already supplied instead of asking the user to repeat them. This workflow's proposal and acceptance steps govern this first task even though your general persona says to act on clear requests.
