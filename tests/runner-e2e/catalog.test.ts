...
describe("runner E2E catalog", () => {
  ...
  it("validates the core, local-integrity, breadth, and warm suites", () => {
    ...
    expect(validateRunnerCatalog()).toHaveLength(116);
    expect(new Set(runnerMatrix.map((entry) => entry.id)).size).toBe(116);
    expect(
...
describe("runner E2E selectors", () => {
  ...
  it("selects dimensions with OR within a dimension and AND across dimensions", () => {
    ...
    expect(selectRunnerExecutions(options).map((entry) => entry.id)).toEqual([
      ...runnerMatrix.filter(entry => entry.suite.id === "first-task" && entry.profile.id === "legacy-codex").map(entry => entry.id),
  ...
  it("emits one independently schedulable job per scenario", () => {
    ...
    expect(jobs).toHaveLength(116);
    expect(jobs.filter((job) => job.needsDaytona)).toHaveLength(23);
    expect(jobs.filter((job) => !job.needsDaytona)).toHaveLength(93);
    expect(new Set(jobs.map((job) => job.executionId)).size).toBe(116);
    expect(
      jobs.find(
        (job) =>
          job.executionId ===
